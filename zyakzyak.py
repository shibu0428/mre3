...
n_values=[10,40,80,100,200, 400, 600, 800, 1000]
...
for n in n_values:
    ...
    print(f"{n:4},{fin_exact_solution[-1]}")
    
    #fin_exact_solution = 2.0
...
scaling_factor_milstein = mean_fins_milstein[0] / (n_values[0]**(-1.0))


n_array = np.array(n_values)
plt.loglog(n_array, scaling_factor_euler * n_array**(-1.0), 'y--', label='-1/2 (Euler)')
plt.loglog(n_array, scaling_factor_milstein * n_array**(-1.0), 'c--', label='-1 (Milstein)')
plt.minorticks_on()
