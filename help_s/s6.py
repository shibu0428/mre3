...
def simulate_and_plot(n_values, trials=1000):
    ...
    for n in n_values:
        ...
        for _ in range(trials):
            # Wiener 過程の増分を作成
            #dW = zscore(np.random.randn(n))*np.sqrt(dt)
            dW =np.random.randn(n)*np.sqrt(dt)
            
            # W_full: shape=(n+1,), 先頭0
        ...
        err_milstein  = abs(mean_fin_milstein - mean_fin_exact) / abs(mean_fin_exact)
...
if __name__ == "__main__":
    n = [10,50,100,300,500,700,1000]
    simulate_and_plot(n, trials=1000)
